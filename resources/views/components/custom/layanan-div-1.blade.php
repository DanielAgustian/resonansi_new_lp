<div class="col-lg-6 col-md-6  bottommargin-lg">

    <div class="feature-box media-box mx-3">
        <div class="fbox-icon d-flex justify-content-center align-items-center">
            <img src="{{ $pic }}" alt="" class="img-section-1-alt">
        </div>
        <div class="fbox-content">
            <h2 class="h4 fw-bold mb-4 text-center">{{ $title }}</h2>
            <div class="lead fw-normal text-black-50 text-justify" style="text-align: justify;">{{ $details }}
            </div>
        </div>
    </div>
    <div class="d-flex justify-content-center mt-3">
        <a href="{{ route('servicePage') }}#{{ $route }}" class="text-center lead fw-bold">
            Selengkapnya >
        </a>
    </div>
    

</div>
